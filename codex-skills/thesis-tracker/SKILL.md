...
This skill is generated from `skills/thesis-tracker.md` so Claude Code, Codex and other harnesses share one canonical workflow.
...
用联网搜索（`web_search`；Claude Code 为 WebSearch）获取当前股价、估值指标（PE/PB/股息率）、最新财报核心数据，用于填写估值锚点。如果已有该公司的 `/investment-research` 或 `/investment-team` 报告，优先从中读取。
...
用联网搜索（`web_search`；Claude Code 为 WebSearch）收集：
1. 最新财报数据（如果有新的季报/年报）
